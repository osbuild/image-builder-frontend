...
test("Create a blueprint with Users customization", async ({
  ...
  await test.step("Create initial valid groups", async () => {
    ...
    await firstGroupInput.type("developers", { delay: 50 });
    ...
    await groupNameInputsAfterAdd.nth(1).clear();
    await groupNameInputsAfterAdd.nth(1).type("admins", { delay: 50 });
  ...
  await test.step("Test group validation errors", async () => {
    ...
    await newGroupInput1.type("@invalid", { delay: 50 });
    ...
    await newGroupInput1.fill("developers");
    await expect(newGroupInput1).toHaveValue("developers");
    await expect(frame.getByText("Invalid group name")).toBeHidden();
    await expect(
  ...
  await test.step("Test group keyboard navigation", async () => {
    ...
    // Tab to Group ID field (read-only)
    await groupNameInput.press("Tab");
    const groupIdInput = frame.getByPlaceholder("Auto-generated").first();
    await expect(groupIdInput).toBeFocused();

    // Tab to remove button
    await groupIdInput.press("Tab");
  ...
  await test.step("Verify blueprint was saved correctly", async () => {
    ...
    // Verify the groups we created are present
    const groupValues = await Promise.all(
      Array.from({ length: groupCount }, (_, i) =>
        groupNameInputs.nth(i).inputValue(),
      ),
    );
    expect(groupValues).toContain("developers");
    expect(groupValues).toContain("managers");
    expect(groupValues).toContain("operators");
