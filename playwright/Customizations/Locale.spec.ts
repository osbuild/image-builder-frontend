...
test('Create a blueprint with Locale customization', async ({
  ...
  await test.step('Select and fill the Locale step', async () => {
    ...
    await frame
      .getByRole('option', { name: 'Afar - Djibouti (aa_DJ.UTF-8)' })
      .click();
    await expect(
      frame.getByRole('button', { name: 'Close Afar - Djibouti (aa_DJ.UTF-8)' })
    ).toBeEnabled();
    await frame.getByPlaceholder('Select a language').fill('aa');
    await expect(
      frame.getByText(
        'Afar - Djibouti (aa_DJ.UTF-8)Language already addedAfar - Eritrea (aa_ER.UTF-8)Afar - Ethiopia (aa_ET.UTF-8)'
      )
    ).toBeAttached();
  ...
  await test.step('Edit BP', async () => {
    ...
    await frame.getByPlaceholder('Select a language').fill('aa');
    await frame
      .getByRole('option', { name: 'Afar - Eritrea (aa_ER.UTF-8)' })
      .click();
    await expect(
      frame.getByRole('button', { name: 'Close Afar - Eritrea (aa_ER.UTF-8)' })
    ).toBeEnabled();
