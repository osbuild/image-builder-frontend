...
test('Create a blueprint with Filesystem customization', async ({
  ...
  await test.step('Check URLs for documentation', async () => {
    ...
    expect(finalUrlManual).toContain(
      'https://docs.redhat.com/en/documentation'
    );
  ...
  await test.step('Fill manually selected partitions', async () => {
    ...
    const rootRow = frame.locator('tr', { hasText: '/' });
    const closeRootButton = rootRow.locator('td:last-of-type >> button');
    ...
    await frame.getByRole('textbox', { name: 'mountpoint suffix' }).click();
    await frame
    ...
    const tmpRow = frame.locator('tr', { hasText: '/tmp' });
    const closeTmpButton = tmpRow.locator('td:last-of-type >> button');
  ...
  await test.step('Edit BP', async () => {
    ...
    const rootRow = frame.locator('tr', { hasText: '/' }).nth(0);
    const closeRootButton = rootRow.locator('td:last-of-type >> button');
    await expect(closeRootButton).toBeDisabled();
  ...
  await test.step('Review imported BP', async (step) => {
    ...
    const tmpRow = frame.locator('tr', { hasText: '/srv' });
    const closeTmpButton = tmpRow.locator('td:last-of-type >> button');
    await expect(closeTmpButton).toBeEnabled();
