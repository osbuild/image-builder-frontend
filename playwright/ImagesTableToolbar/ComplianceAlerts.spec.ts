...
test('Compliance alerts - lint warnings display', async ({ page, cleanup }) => {
  ...
  await test.step('Edit compliance policy to remove firewalld package', async () => {
    ...
    const rulesContent = page.locator('[role="tabpanel"], .pf-v6-c-tab-content, [data-ouia-component-type*="Rules"], [data-ouia-component-type*="Tailoring"]');
    const rulesContentVisible = await rulesContent.first().isVisible({ timeout: 10000 }).catch(() => false);
    ...
    try {
      const searchInputByPlaceholder = page.getByPlaceholder(/search|filter|type to filter|filter by name/i);
      const placeholderCount = await searchInputByPlaceholder.count();
      if (placeholderCount > 0) {
        const firstInput = searchInputByPlaceholder.first();
        const isVisible = await firstInput.isVisible({ timeout: 3000 }).catch(() => false);
        if (isVisible) {
    ...
    if (!searchInput) {
      try {
        const searchInputByLabel = page.locator('input[aria-label*="search" i], input[aria-label*="filter" i]');
    ...
    if (!searchInput) {
      try {
        const toolbarInputs = page.locator('.pf-v6-c-toolbar input[type="text"], .pf-v6-c-toolbar__content input, [data-ouia-component-type="PF4/TextInput"]');
    ...
    if (searchInput) {
      ...
      console.log('Could not find search input, trying to find firewalld directly');
    }

    await page.waitForTimeout(2000);

    // Try to find and uncheck a package - try multiple packages
    const packagesToTry = ['firewalld', 'aide', 'audit', 'cronie', 'sudo', 'logrotate', 'rsyslog'];
    ...
    for (const packageName of packagesToTry) {
        ...
        if (textCount > 0) {
          ...
          for (let i = 0; i < textCount; i++) {
            ...
            if (isVisible) {
              ...
              const hasRow = await parent.locator('tr, [role="row"]').count().catch(() => 0);
              if (hasRow > 0 || await parent.getByRole('row').count().catch(() => 0) > 0) {
                packageElement = elem;
                break;
              }
            }
      ...
      if (!packageElement) {
        try {
          const packageRow = page.getByRole('row', { name: new RegExp(packageName, 'i') });
      ...
      if (!packageElement) {
        try {
          const packageCell = page.getByRole('gridcell', { name: new RegExp(packageName, 'i') });
      ...
      if (!packageElement) {
        try {
          ...
          const isVisible = await packageLocator.isVisible({ timeout: 2000 }).catch(() => false);
          if (isVisible) {
      ...
      if (packageElement) {
        ...
        for (let level = 0; level < 6; level++) {
          const row = currentElement.locator('..').locator('tr, [role="row"]').first();
          ...
          try {
            ...
            if (checkboxVisible) {
              ...
              if (isChecked) {
                ...
                unchecked = true;
                console.log(`Successfully unchecked ${packageName} via checkbox`);
              }
            }
          } catch (e) {
            // Try toggle
            ...
            try {
              ...
              if (toggleVisible) {
                ...
                if (isChecked) {
                  await toggle.click();
                  unchecked = true;
                  console.log(`Successfully unchecked ${packageName} via toggle`);
                }
              }
            } catch (e) {
              // Continue
            ...
            try {
              ...
              if (checkboxVisible) {
                ...
                if (isChecked) {
                  await checkbox.uncheck();
                  unchecked = true;
                  console.log(`Successfully unchecked ${packageName} via checkbox (traverse)`);
                  break;
                }
              }
            } catch (e) {
            ...
            try {
              ...
              if (toggleVisible) {
                ...
                if (isChecked) {
                  await toggle.click();
                  unchecked = true;
                  console.log(`Successfully unchecked ${packageName} via toggle (traverse)`);
                  break;
                }
              }
            } catch (e) {
        ...
        if (unchecked) {
          ...
          console.log(`Found ${packageName} but could not uncheck it - might already be unchecked or not selectable`);
        }
      }
    }

    if (!packageUnchecked) {
      console.log(`Could not find or uncheck any of the expected packages (${packagesToTry.join(', ')}) in compliance policy rules`);
    }
