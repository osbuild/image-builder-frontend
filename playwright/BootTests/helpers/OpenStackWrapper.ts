...
export class OpenStackWrapper {
  ...
  public async launchInstance(): Promise<void> {
    ...
    try {
      console.log(
        `Launching instance ${this.imageName} from image ${this.imageName}`,
      );
      const output = await OpenStackWrapper.execCommand(
        `openstack server create -f json --image="${this.imageName}" --flavor="g.standard.small" --network="shared_net_1" --security-group="default" --key-name="${this.keyName}" ${this.imageName}`,
      );
      const instance = JSON.parse(output);
      // Expect the instance started building
      if (instance.status !== 'BUILD') {
        throw new OpenStackError(
          `Instance ${this.imageName} does not have expected status 'BUILD', but '${instance.status}'`,
        );
      }
      // Wait until the instance is running (in active state)
      for (let i = 0; i < retries; i++) {
        const output = await OpenStackWrapper.execCommand(
          `openstack server show -f json ${this.imageName}`,
        );
        const instance = JSON.parse(output);
