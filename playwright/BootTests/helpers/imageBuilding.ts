...
export const buildImage = async (page: Page) => {
    ...
    while (true) {
      if (await page.getByText('Ready').isVisible() || await page.getByText('Expires in').isVisible()) {
      ...
      if (timeSpentBuilding % (60000*5) === 0) { // Log only every 5 minutes
        console.log(`Waiting for image to be ready (Time spent: ${timeSpentBuilding / 60000}m)`);
      }
    }
}
