...
export const selectTarget = async (page: Page | FrameLocator, target: 'qcow2' | 'iso' | 'wsl' | 'ova' | 'vmdk') => {
    /**
     * Select the target.
     * @param page - the page object
    ...
    switch (target) {
        ...
        case 'wsl':
            ...
            break;
        case 'ova':
            await page.getByRole('checkbox', { name: 'VMware vSphere - Open virtualization format' }).click();
            break;
        case 'vmdk':
            await page.getByRole('checkbox', { name: 'VMware vSphere - Virtual disk' }).click();
            break;
    }
