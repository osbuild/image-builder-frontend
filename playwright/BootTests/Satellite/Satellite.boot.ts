...
const validRegistrationCommand = `set -o pipefail && curl --silent --show-error 'https://localhost/register?activation_keys=my-key&download_utility=curl&location_id=2&organization_id=1&update_packages=false' --header 'Authorization: Bearer fake.eyJleHAiOjk5OTk5OTk5OTl9.fake' | bash`;
const validCertificate = `-----BEGIN CERTIFICATE-----test;-----END CERTIFICATE-----`;
...
test('Satellite registration boot integration test', async ({
  ...
  await test.step('Verify Satellite registration was run', async () => {
    ...
    expect(output).toContain(
      `curl --silent --show-error 'https://localhost/register?activation_keys=my-key&download_utility=curl&location_id=2&organization_id=1&update_packages=false' --header 'Authorization: Bearer fake.eyJleHAiOjk5OTk5OTk5OTl9.fake' | bash`,
    );
  });

  await test.step('Verify the certificate file was correctly uploaded', async () => {
    const [exitCode] = await image.exec(
      'ls /etc/pki/ca-trust/source/anchors/ | grep -i satellite',
    );
    expect(exitCode).toBe(0);
  });
});
