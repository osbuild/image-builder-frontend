...
const Repositories = () => {
  ...
  useEffect(() => {
    if (isTemplateSelected && reposInTemplate.length > 0) {
      ...
      addSelected(customReposInTemplate);
    }
  }, [selectedTemplateData, reposInTemplate]);
  ...
  if (!isTemplateSelected) {
              ...
              <BulkSelect
                ...
                isDisabled={
                  ...
                  contentList.every(
                    (repo) =>
                      repo.url &&
                      isRepoDisabled(repo, selected.has(repo.url))[0]
                  )
                }
              />
            </ToolbarItem>
    ...
    return (
      <>
        <Alert
          variant="info"
          title="The repositories seen below are from the selected content template and have been added automatically. 
              If you do not want these repositories in your image, you can modify the content template or choose another snapshot option."
        />
        <Grid>
          <Panel>
            <PanelMain>
