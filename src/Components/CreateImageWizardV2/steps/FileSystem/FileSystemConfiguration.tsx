...
type FileSystemConfigurationStepFooterPropTypes = {
  setNextDisable(isNextDisable: boolean): void;
  isNextDisable: boolean;
};

export const FileSystemStepFooter: React.FunctionComponent<
  FileSystemConfigurationStepFooterPropTypes
> = ({ setNextDisable, isNextDisable }) => {
  ...
  const onValidate = () => {
    dispatch(changeIsNextButtonPristine());
    if (!isValid) {
      setNextDisable(true);
    } else {
      goToNextStep();
  ...
  return (
    <WizardFooterWrapper>
      <Button onClick={onValidate} isDisabled={isNextDisable}>
        Next
      </Button>
