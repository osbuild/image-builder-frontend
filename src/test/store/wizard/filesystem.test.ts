...
describe('filesystem reducers', () => {
  ...
  describe('changePartitionOrder', () => {
    it('should reorder partitions according to provided order', () => {
      const stateWithPartitions: wizardState = {
        ...initialState,
        fileSystem: {
          partitions: [
      ...
      const result = wizardReducer(
        stateWithPartitions,
        changePartitionOrder(['p3', 'p1', 'p2']),
      );

      expect(result.fileSystem.partitions[0].id).toBe('p3');
      expect(result.fileSystem.partitions[1].id).toBe('p1');
      expect(result.fileSystem.partitions[2].id).toBe('p2');
    });

    it('should sort items not in order list by their original indexOf position (-1)', () => {
      const stateWithPartitions: wizardState = {
        ...initialState,
        fileSystem: {
          partitions: [
      ...
      // Items not in the order array get indexOf -1, so they sort before listed items
      const result = wizardReducer(
        stateWithPartitions,
        changePartitionOrder(['p2']),
      );

      // p1 and p3 have indexOf -1, p2 has indexOf 0
      // So p1 and p3 come before p2 (both have -1, maintaining relative order)
      expect(result.fileSystem.partitions[2].id).toBe('p2');
    });
  });
