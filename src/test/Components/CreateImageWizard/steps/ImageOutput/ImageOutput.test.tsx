...
describe('Network installer target', () => {
  ...
  test('does not render Network - Installer when feature flag is disabled', async () => {
    vi.mocked(useFlag).mockImplementation(() => false);
    ...
    expect(
      screen.queryByRole('checkbox', { name: /Network - Installer/i }),
    ).not.toBeInTheDocument();
  });
  ...
  test('selecting network-installer only shows locale and details steps', async () => {
    ...
    const stepLabels = stepButtons.map((button) => button.textContent.trim());

    const expectedSteps = ['Image output', 'Locale', 'Details', 'Review'];
    expect(stepLabels).toEqual(expectedSteps);
  });
