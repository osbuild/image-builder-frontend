    ...
    queryFn: onPremQueryHandler(
      async ({ queryArgs: { id: filename } }) => {
        ...
        for (const ir of blueprint.image_requests) {
          ...
          const id = uuidv4();
          const bpPath = path.join("/var/cache/cockpit-image-builder", `cockpit-image-builder-${id}.json`);
          await cockpit.file(bpPath, { superuser: "require" }).replace(JSON.stringify(bpOnPrem));
          await cockpit.spawn([
            "systemd-run",
            "--setenv",
            "HOME=" + user.home,
    ...
    queryFn: onPremQueryHandler(async ({ queryArgs }) => {
      ...
      for (const bpEntry of entries) {
        ...
        if (upresEntry !== undefined) {
          ...
          if (status.image_status.status === "success" && status.image_status.upload_status.status === "failure") {
            status.image_status.status = "failure";
            status.image_status.error = {
              id: 28,
          ...
          status.image_status.error = {
            id: 28,
            reason: "image-builder process is not running and no upload result found",
          };
        }
