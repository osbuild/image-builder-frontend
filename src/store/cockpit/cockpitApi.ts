      ...
      >({
        queryFn: async ({ distribution, profile }) => {
          try {
            ...
            // the profile description is contained under the profile info
            // command of oscap, it is not available in the generated blueprint
            const description = result
              .split('\n')
              .filter((s) => s.includes('Description: '))[0]
              .split('Description: ')[1];
